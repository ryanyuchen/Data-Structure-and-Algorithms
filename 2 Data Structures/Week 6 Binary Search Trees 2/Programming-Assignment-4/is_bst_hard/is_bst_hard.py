...
def IsBinarySearchTree(tree):
  ...
  for i in range(0, len(treevec)-1):
      if tree.key[treevec[i+1]] < tree.key[treevec[i]]:
          return False
      if tree.key[treevec[i]] == tree.key[treevec[i+1]] and tree.left[treevec[i+1]] == treevec[i]:
          return False
  return True
