...
public:
    AStar(int n, Adj adj, Adj cost, vector<pair<Len,Len>> xy)
        : n_(n), adj_(adj), cost_(cost), distance_(vector<Len>(n_, INFINITY)), visited_(n), xy_(xy), p_(vector<Len>(n_, -1))
    {workset_.reserve(n);}
    ...
    Len Potential(int m, int n) {
		if(p_[m] == -1) {
			pair<Len, Len> mCoord = xy_[m];
			pair<Len, Len> nCoord = xy_[n];
			p_[m] = sqrt((mCoord.first - nCoord.first)*(mCoord.first - nCoord.first) + (mCoord.second - nCoord.second)*(mCoord.second - nCoord.second));
		}
		return p_[m];
	}
	...
	void Process(Queue& q, int v, int t, vector<vector<int>> &adj, vector<vector<int>> &cost) {
		for (int i = 0; i < adj[v].size(); ++i) {
			int u = adj[v][i];
			if (visited_[u] != true) {
				int w = cost[v][i];
				visit(q, u, distance_[v] + w, Potential(u, t));
			}
		}	
	}
