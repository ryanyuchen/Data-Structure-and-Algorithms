...
class AStar:
    def __init__(self, n, adj, cost, x, y):
        ...
        self.x = x
        self.y = y
        self.p = {}     # potential function using Euclidean Distance
    ...
    def Potential(self, m, n):
        if m not in self.p:
            mCoord = (self.x[m], self.y[m])
            nCoord = (self.x[n], self.y[n])
            self.p[m] = math.sqrt((mCoord[0] - nCoord[0])**2 + (mCoord[1] - nCoord[1])**2)
        return self.p[m]
