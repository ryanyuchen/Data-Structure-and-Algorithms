...
public:
    ...
    int ExtraMin(Queue& q, int side){
        pair<Len, int> p = q[side].top();
        q[side].pop();
        return p.second;
    }
    
    void Process(Queue& q, int side, int v, vector<vector<int>> &adj, vector<vector<int>> &cost){
        for (int i = 0; i < adj[v].size(); i++){
            int u = adj[v][i];
            int w = cost[v][i];
            visit(q, side, u, distance_[side][v] + w);
        }
    }
    ...
    Len query(int s, int t) {
        ...
        while (!q[0].empty() && !q[1].empty()){
            ...
            if (visited_[v] == true) return ShortestPath(v);
            visited_[v] = true;
            
            int vr = ExtraMin(q, 1);
            Process(q, 1, vr, adj_[1], cost_[1]);
            if (visited_[vr] == true) return ShortestPath(vr);
            visited_[vr] = true;
        }
